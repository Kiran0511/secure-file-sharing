...
import { User, Key, Upload, LogOut, ArrowUpCircle } from "lucide-react"
import { Button } from "@/components/ui/button"
import { ReactNode, useEffect, useState } from "react"

export default function UserSidebarLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col sm:flex-row items-stretch justify-center p-2 sm:p-4 relative overflow-hidden bg-[#121212]">
      ...
      <div className="w-full max-w-4xl flex flex-col sm:flex-row relative z-10 mx-auto">
        {/* Sidebar */}
        <aside className="w-full sm:w-64 bg-gray-800/90 border-b sm:border-b-0 sm:border-r border-gray-700 rounded-t-xl sm:rounded-l-xl sm:rounded-tr-none p-4 sm:p-6 flex flex-row sm:flex-col gap-4 sm:gap-8 shadow-xl">
          <div className="flex flex-row sm:flex-col items-center gap-2 sm:gap-2">
            <User className="w-8 h-8 sm:w-10 sm:h-10 text-blue-400 mb-0 sm:mb-2" />
            <span className="text-base sm:text-lg font-semibold text-gray-100 truncate">{email || "User"}</span>
            <Button
              variant="outline"
              size="sm"
              className="ml-auto sm:ml-0 mt-0 sm:mt-2 border-gray-600 text-gray-300 hover:text-white hover:border-blue-500"
          ...
          <nav className="flex flex-row sm:flex-col gap-2 sm:gap-4 flex-1 justify-end sm:justify-start">
            <Button
              variant="ghost"
            ...
            <Button
              variant="ghost"
              className="justify-start text-gray-300 hover:text-blue-400 w-full"
              onClick={() => router.push("/users/myuploads")}
            >
